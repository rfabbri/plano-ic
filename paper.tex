...
indústria automobilística e metal-mecânica.
...
%   \includegraphics[width=1.0\linewidth]{figs/3d-curve-sketch/system-diagram.eps}
   \includegraphics[width=1.0\linewidth]{figs/mega-system-drawing-pt.eps}
\end{center}
\vspace{-0.7truecm}
   \caption{%
   Uma visão geral do sistema automático de reconstrução 3D envisado pela nossa
   pesquisa. A entrada consite em um conjunto de $N$ imagens (fotos ou vídeo)
   tiradas de uma imagem, com $N$ sendo ao menos 3, mas típicamente muito maior,
   e a saída é um conjunto de curvas e superfícies reconstruídas, junto com
   modelos de câmeras (posição, rotação, \emph{focallength}, etc) otimizados.
   Na prática, há o feedback entre os módulos.$^1$
   }\label{fig:system:diagram}
\end{figure}
\footnotetext[1]{Os
   termos na figura não foram traduzidos pois contêm um número considerável de
   jargões que atualmente não possuem equivalentes satisfatórios em português.}
...
\section{Equipe, Recursos Necessários e Orçamento}

Para este projeto, pretende-se orientar ao menos 2 alunos de mestrado, sendo
necessários, assim, recursos para o trabalho de ao menos 3 pesquisadores
(orientador e alunos). Os principais equipamentos para o trabalho nesta área são
os \emph{computadores de alto desempenho}, os quais são imprescindíveis para
processamento gráfico, seguido de periféricos tais como webcams e impressoras.
Portanto, neste pedido à FAPERJ estamos prevendo a compra de 2 laptops de ponta
(necessários além do que já possuímos no laboratório), capazes de executar os
experimentos e programas avançados de reconstrução 3D, tanto no laboratório como
em campo para a aquisição em tempo real. O orçamento está detalhado em uma
planilha eletrônica em anexo. Também anexamos um orçamento detalhado através do
site da Apple.

Conforme o orçamento em anexo, os laptops de processamento gráfico custarão
$2\times US\$4.176,00$ dólares que, de acordo com a taxa de câmbio de 1/4/2012 resulta em 
R\$15.234,04 reais. A taxa de importação prevista no orçamento é de 18\%,
resultando em um custo adicional de R\$2,742.13 reais. Assim, \textbf{o total
requisitado é de 17.976,17 reais.}
